...
def _rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map: dict[str, str] = {
        ...
        "ST \nBOYS": "ST_BOYS",
        "ST \nGIRLS": "ST_GIRLS",
        # Treat EWS columns as gendered cutoffs for consistency.
        "EWS \nGEN OU": "EWS_BOYS",
        "EWS \nGIRLS OU": "EWS_GIRLS",
    }
...
def load_college_dataset(path: str | Path = "cleaned.xlsx") -> CollegeDataset:
    ...
    df = df.dropna(subset=["Institute_Name", "Branch_Name", "Fee"])
...
def available_college_types(dataset: CollegeDataset) -> list[str]:
    if "College_Type" not in dataset.df.columns:
        return []
    vals = dataset.df["College_Type"].dropna().astype(str).str.strip()
    return sorted({v for v in vals.tolist() if v})


def available_co_ed(dataset: CollegeDataset) -> list[str]:
    if "Co_Education" not in dataset.df.columns:
        return []
    vals = dataset.df["Co_Education"].dropna().astype(str).str.strip()
    return sorted({v for v in vals.tolist() if v})
